...
def filter_on_date(df, start, end, date_col="DATE"):
    ...
    # Ensure start and end are pd.Timestamp
...
def main(start_data, end_data):
    ...
    ###### SET UP PAGE ######
    icon_path = os.path.join(".", "raw", "greenrock.png")
    st.set_page_config(page_title="Greenrock", page_icon=icon_path,
...
if __name__ == "__main__":
    ...
    if f"{start_data}_to_{end_data}" not in os.listdir("C:\\Users\\SAISH\\Desktop\\MP\\Data"):
...
if st.button('Future Performance'):
    # Use st.write to create a clickable hyperlink
    st.write(f'<a href="{url}" target="_blank">Click here to open the URL</a>', unsafe_allow_html=True)
